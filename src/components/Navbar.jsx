  ...
  return (
    ...
    <nav className="bg-white">
      ...
      >
        {/* Logo */}
        <Link to={"/"}>
          <img src={logo} alt="Traveladvisor" className="w-[80px]" />
        </Link>
        {/*  */}

        <ul className="hidden mmd:flex space-x-1 items-center">
          ...
          {/* --- */}
          {loggedIn?.data?.user?.role === "user" && (
            <NavLink to={"list"}>
              <li className="rounded-full hover:bg-gray-200 py-2 px-3 cursor-pointer">
                <p className="flex font-medium items-center">
                  <svg viewBox="0 0 24 24" className="w-6 h-6 mr-2">
                    <path d="M12 2c-5.5 0-10 4.5-10 10s4.5 10 10 10 10-4.5 10-10-4.5-10-10-10zm0 18c-4.4 0-8-3.6-8-8s3.6-8 8-8 8 3.6 8 8-3.6 8-8 8zm0-14c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm0 10c-2.2 0-4-1.8-4-4s1.8-4 4-4 4 1.8 4 4-1.8 4-4 4z"></path>
                  </svg>
                  Services
                </p>
              </li>
          ...
          {loggedIn?.data?.user?.role === "serviceProvider" && (
          ...
          {loggedIn && (
            <NavLink to={"profile"}>
              <li className="rounded-full  py-2 px-3 ">
                <p className="flex font-medium  ">
                  <TbUserCircle className="w-6 h-6 mr-2 rounded-full" />
                  Welcome, {loggedIn?.data?.user?.name}
                </p>
              </li>
            </NavLink>
          )}
          {loggedIn && (
            <Link>
              <li className="rounded-full py-2 px-3 ">
                <button
                  className="mx-3 bg-white text-black outline outline-offset-2 hover:bg-secondary rounded-full py-2 px-3  cursor-pointer "
          ...
          {!loggedIn && (
            <Link className="flex">
              <Link to={"/signup"}>
                <li className="rounded-full py-2 px-3 cursor-pointer hover:bg-secondary">
                  <p className="flex font-medium items-center">Signup</p>
                </li>
              </Link>
              <Link to={"/login"}>
                <li className="rounded-full bg-black text-white py-2 px-3 cursor-pointer">
                  <p className="flex font-medium items-center">Login</p>
                </li>
              </Link>
            </Link>
          )}
      ...
      {isMenuToggled && (
        <div className="flex flex-col mmd:hidden bg-white shadow-md absolute animate-slide-in right-0">
          ...
          <Link to={"/login"}>
            <p className="flex font-medium items-center cursor-pointer px-4 py-3 outline outline-secondary">
              Signup
            </p>
          </Link>
          <Link to={"/signup"}>
